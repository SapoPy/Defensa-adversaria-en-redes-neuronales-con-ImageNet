...
def image_gradient_targeted(model, img_norm, target_idx, device):
    """
    Gradiente de pérdida hacia una clase objetivo.
    Retorna gradiente ∂L/∂x (en espacio normalizado).
    """
...
def show_rfgsm_comparison(img_pixel, adv_pixel, save_fig=False, eps=0.05, out_dir="graficos"):
    ...
    axs[2].imshow(adv_pixel.permute(1,2,0).numpy()); axs[2].set_title("Adversaria dirigida"); axs[2].axis("off")
    plt.suptitle(f"R-FGSM dirigido (ε={eps})")
    plt.tight_layout()
    if save_fig:
        os.makedirs(out_dir, exist_ok=True)
        fname = os.path.join(out_dir, f"rfgsm_dirigido_eps_{eps:.4f}.png")
...
# R-FGSM Dirigido
# -------------------------
def generar_rfgsm_dirigido(model, val_dataset, target_idx, output_dir="RFGSM_dirigido",
                            eps=0.05, seed_std=None, device=None,
                            show_examples_n=3, save_examples=True):
    """
    Versión dirigida del ataque R-FGSM:
    - Se añade ruido aleatorio inicial (pixel-space)
    - Luego se mueve la imagen en dirección que *aumenta la probabilidad de target_idx*
    """
    ...
    for idx in range(total):
        ...
        # Signo NEGATIVO para ataque dirigido
        eps_norm = (eps_cpu / std_cpu)
        adv_norm = img_rand_norm - eps_norm * grad_norm.sign()

        # Limitar al espacio válido
        ...
        save_path = os.path.join(class_dir, f"img_{idx:06d}_rfgsm_target_{target_idx}.png")
    ...
    print(f"R-FGSM dirigido completado. Imágenes en: {output_dir}")
    return output_dir
...
if __name__ == "__main__":
    ...
    generar_rfgsm_dirigido(
        model, val_ds, target_idx=TARGET_IDX,
        output_dir=f"RFGSM_dirigido_target_{TARGET_IDX}",
        eps=EPS, seed_std=SEED_STD,
